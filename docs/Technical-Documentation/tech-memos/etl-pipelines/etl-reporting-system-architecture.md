...
The pipeline registry remains the logical DAG definition. The runner validates the DAG, resolves dependencies, creates `ETLPipelineRun` and `ETLNodeRun` rows, then computes dependency layers. It queues one executable layer at a time instead of building one large Canvas graph for the whole run.
