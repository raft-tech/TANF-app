...
* `GHCR_PUBLISH_USERNAME`: GitHub machine user allowed to publish the current organization's `tdp-keycloak` package
* `GHCR_PUBLISH_TOKEN`: machine-user token limited to `write:packages`
* `GHCR_PULL_USERNAME`: GitHub machine user recorded on the Cloud Foundry Keycloak Docker package
* `GHCR_PULL_TOKEN`: separate machine-user token limited to `read:packages`
