...
| `GO_PARSER_METRICS_ENABLED` | `metrics.enabled` / `--metrics.enabled` | Enables the Prometheus metrics exporter for non-local server modes |
...
The parser exports Prometheus text-format metrics for long-running server modes (`celery`, `grpc`, and `http`). Local mode is a one-shot file processing command, so it does not start the metrics server.
...
- pipeline stage latency histograms for `setup`, `decoding`, `parsing`, `group_validation`, `record_validation`, `field_validation`, `routing`, and `flush`
