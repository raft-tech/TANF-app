...
query_template = """
SELECT {fields}
    canonical_section.name::varchar(32) AS section,
    data_files.version,
    data_files.year,
    data_files.quarter,
    ...
    canonical_program.code::varchar(32) AS program_type
...
INNER JOIN
    ...
    INNER JOIN (
        SELECT
            ...
            is_program_audit,                                              -- Separate standard and audit submissions
            MAX(version) AS version                                        -- Get the maximum version for each group
        FROM
            data_files_datafile                                            -- Subquery table
        GROUP BY
            stt_id, section_ref_id, year, quarter, is_program_audit        -- Group by columns
    ) most_recent
        ...
        AND data_files.is_program_audit = most_recent.is_program_audit
    INNER JOIN
        data_files_section canonical_section                               -- Join canonical section metadata
        ON data_files.section_ref_id = canonical_section.id
    INNER JOIN
        data_files_program canonical_program                               -- Join canonical program metadata
        ON canonical_section.program_id = canonical_program.id
    INNER JOIN
