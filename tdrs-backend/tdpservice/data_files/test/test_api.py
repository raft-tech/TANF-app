...
class DataFileAPITestBase:
    ...
    def test_datafile(self, stt_user, stt):
        ...
        test_datafile = util.create_test_datafile(
            "small_incorrect_file_cross_validator.txt", stt_user, stt
        )
    ...
    def post_data_file(self, api_client, data_file_data):
        ...
        return api_client.post(self.root_url, data_file_data, format="multipart")
    ...
    def get_data_file_file(self, api_client, data_file_id):
        ...
        return api_client.get(f"{self.root_url}{data_file_id}/")
...
class TestDataFileAPIAsDataAnalyst(DataFileAPITestBase):
    ...
    def test_download_fra_error_report_file_for_own_stt(
        self, request, api_client, file, dfs
    ):
    ...
    def test_download_error_report_ssp_file_for_own_stt(
        self, api_client, test_ssp_datafile, dfs
    ):
    ...
    def test_download_data_file_file_rejected_for_other_stt(
        self, api_client, data_file_data, other_stt, user
    ):
...
class TestDataFileAPIAsInactiveUser(DataFileAPITestBase):
    ...
    def test_data_files_inactive_user_not_allowed(
        self, api_client, inactive_user, data_file_data
    ):
...
def test_list_uses_canonical_classification_when_legacy_values_drift(
    ...
    response = api_client.get(
        f"/v1/data_files/?stt={data_analyst.stt.id}&file_type=ssp-moe"
    )

    assert response.status_code == status.HTTP_200_OK
    assert [item["id"] for item in response.data] == [data_file.id]
    assert response.data[0]["program_type"] == "SSP"
    assert response.data[0]["section"] == "Active Case Data"
...
class TestDataFileQuerysetFiltering:
    ...
    Program integrity audit applies to TANF and Tribal TANF files. SSP and FRA
    files do not have audit variants.
