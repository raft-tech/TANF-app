...
def parse(data_file_id):
    ...
    logger.info(f"DataFile parsing started for file {data_file.id}")
    errors = parse_datafile(data_file)
    logger.info(f"DataFile parsing finished for file {data_file.id} with {len(errors)} errors: {errors}")
