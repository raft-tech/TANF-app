    ...
    def get_suggested_decoder(cls, raw_file):
        ...
        if not len(raw_file):
            match extension:
                case ".csv":
                    logger.warning(
                        "File was empty. Returning CSV decoder based on extension."
                    )
                    return Decoder.CSV
                case ".xlsx":
                    logger.warning(
                        "File was empty. Returning XLSX decoder based on extension."
                    )
                    return Decoder.XLSX
                case _:
                    logger.warning(
                        "File was empty. Returning UTF8 decoder based on extension."
                    )
                    return Decoder.UTF8
        ...
        if encoding is not None and (encoding == "ascii" or encoding == "UTF-8"):
            ...
            if "csv" in extension:
                logger.info(
                    f"Returning CSV decoder with a confidence score of {confidence}"
                )
                return Decoder.CSV
            logger.info(
                f"Returning UTF-8 decoder with a confidence score of {confidence}"
            )
            return Decoder.UTF8
        else:
            try:
                ...
                if most_confident.extension == "xlsx":
                    logger.info(
                        f"Returning XLSX decoder with a confidence score of {most_confident.confidence}"
                    )
                    return Decoder.XLSX
