...
def generate_t2_t3_t5_hashes(line, record):
    ...
    return hash(line), hash(record.RecordType + str(record.RPT_MONTH_YEAR) + str(record.CASE_NUMBER or '') +
                            str(record.FAMILY_AFFILIATION) + str(record.DATE_OF_BIRTH or '') + str(record.SSN or ''))
